...
const { handleParamsValue, ArrayToObject, Decrypt } = require('../../common/utils.js');
...
class openController extends baseController {
  ...
  /**
   * RAP接口
   * @interface /open/interface_get
   * @method GET
   * @category open
   * @param {Number} id 接口id，不能为空
   * @returns {Object}
   * **/
  async interfaceGet(ctx) {
    let params = ctx.params;
    if (!params.id) {
      return (ctx.body = yapi.commons.resReturn(null, 400, '接口id不能为空'));
    }

    let decodeId = Decrypt(params.id)
    if (!/^\d+$/.test(decodeId)) {
      return (ctx.body = yapi.commons.resReturn(null, 400, '接口id无效'));
    }

    try {
      let result = await this.Model.get(decodeId);
      if(this.$tokenAuth){
      ...
      if (!result) {
        return (ctx.body = yapi.commons.resReturn(null, 490, 'id对应的接口不存在'));
      }
